...
if [ $# -eq 0 ]; then
    echo "You have to enter the instance id as an argument e.g. ./init.sh <INSTANCE_ID>"
    exit 1
fi

if [ -d "/opt/ZTNA/instances/$INSTANCE_ID" ]; then
    echo "The instance has already been configured!"
    exit 1
fi

mkdir /opt/ZTNA/instances/$INSTANCE_ID
cd /opt/ZTNA/instances/$INSTANCE_ID

NETBIRD_DOMAIN="$INSTANCE_ID.yourDomain.com"
...
set +e
while true; do
    if $DOCKER_COMPOSE_COMMAND exec -T crdb$INSTANCE_ID curl -sf -o /dev/null 'http://localhost:8080/health?ready=1'; then
    break
    fi
    echo -n " ."
    sleep 5
...
$DOCKER_COMPOSE_COMMAND up -d crdb$INSTANCE_ID
echo ""

echo -n "Waiting cockroachDB  to become ready "
wait_crdb
echo '$DOCKER_COMPOSE_COMMAND exec -T crdb$INSTANCE_ID /bin/bash -c "cp /cockroach/certs/* /zitadel-certs/ && cockroach cert create-client --overwrite --certs-dir /zitadel-certs/ --ca-key /zitadel-certs/ca.key zitadel_user && chown -R 1000:1000 /zitadel-certs/"'
$DOCKER_COMPOSE_COMMAND exec -T crdb$INSTANCE_ID /bin/bash -c "cp /cockroach/certs/* /zitadel-certs/ && cockroach cert create-client --overwrite --certs-dir /zitadel-certs/ --ca-key /zitadel-certs/ca.key zitadel_user && chown -R 1000:1000 /zitadel-certs/"
...
export SERVICE_USER_CLIENT_ID
export SERVICE_USER_CLIENT_SECRET
}
...
if [ "$DOMAIN" == "netbird.yourDomain.com" ]; then
    echo "The NETBIRD_DOMAIN cannot be netbird.yourDomain.com" > /dev/stderr
    retrun 1
fi
return 0
}
...
if [ -f zitadel.env ]; then
    ...
    echo "  rm -f docker-compose.yml zitadel.env dashboard.env machinekey/zitadel-admin-sa.token management.json"
...
$DOCKER_COMPOSE_COMMAND up -d zitadel$INSTANCE_ID
init_zitadel
...
echo -e "\nDone!\n"
echo "Username=$ZITADEL_ADMIN_USERNAME" >> .env
echo "Password=$ZITADEL_ADMIN_PASSWORD" >> .env

concatenated_string="${SERVICE_USER_CLIENT_ID}:${SERVICE_USER_CLIENT_SECRET}"
base64_key=$(echo -n "$concatenated_string" | base64 | tr -d '[:space:]')

echo "AuthKey=$base64_key" >> .env
payload="{\"AuthKey\":\"$(printf %s "$base64_key")\"}"

#Sending webhook with Authkey as payload
curl -X POST -d "$payload" -H "Content-Type: application/json" "https://xxx.yourDomain.com/deploy/netbird/$INSTANCE_ID/end"
}
...
cat <<EOF
{
    ...
    "TURNConfig": {
        "Turns": [
            {
                "Proto": "udp",
                "URI": "turn:turn1.xxx.examples.com:3478",
                "Username": "self",
                "Password": "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
            },
            {
                "Proto": "udp",
                "URI": "turn:turn2.xxx.examples.com:3478",
                "Username": "self",
                "Password": "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
            }
        ],
    ...
    "DeviceAuthorizationFlow": {
        "Provider": "none",
        "ProviderConfig": {
        "Audience": "netbird-service-account"
    }},
...
ZITADEL_DATABASE_COCKROACH_HOST=crdb$INSTANCE_ID
...
services:
    ...
    signal$INSTANCE_ID:
        ...
        networks: [netbird]
    # Management
    management$INSTANCE_ID:
        image: netbirdio/management:latest
        restart: unless-stopped
        networks: [netbird]
        volumes:
            - netbird_management$INSTANCE_ID:/var/lib/netbird
    ...
    zitadel$INSTANCE_ID:
        ...
        volumes:
            ...
            - netbird_zitadel${INSTANCE_ID}_certs:/crdb-certs:ro
    # CockroachDB for zitadel
    crdb$INSTANCE_ID:
        restart: 'always'
        networks: [netbird]
        image: 'cockroachdb/cockroach:v22.2.2'
        command: 'start-single-node --advertise-addr crdb$INSTANCE_ID'
        volumes:
            - netbird_crdb${INSTANCE_ID}_data:/cockroach/cockroach-data
            - netbird_crdb${INSTANCE_ID}_certs:/cockroach/certs
            - netbird_zitadel${INSTANCE_ID}_certs:/zitadel-certs
        healthcheck:
...
volumes:
    netbird_management$INSTANCE_ID:
    netbird_crdb${INSTANCE_ID}_data:
    netbird_crdb${INSTANCE_ID}_certs:
    netbird_zitadel${INSTANCE_ID}_certs:
