...
type sqlRepository struct {
	table *db.Table[AccessLogEntry]
}

func NewRepository(conn *db.Conn) Repository {
	return &sqlRepository{table: db.NewTable[AccessLogEntry](conn)}
}

func (r *sqlRepository) Create(ctx context.Context, tx *db.Tx, entry *AccessLogEntry) error {
...
func (r *sqlRepository) ListByAccount(ctx context.Context, tx *db.Tx, lockStrength db.LockingStrength, accountID string, filter AccessLogFilter) ([]*AccessLogEntry, int64, error) {
...
func applyFilters(query *db.Query, filter AccessLogFilter) *db.Query {
