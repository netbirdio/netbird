...
// An unvalidated row must never yield a cluster: the account has not proven it
// controls the name, so nothing may be routed for it.
...
// A more specific unvalidated row must not shadow a validated parent domain the
// account already proved it owns.
