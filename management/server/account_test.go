...
func createManager(t testing.TB) (*DefaultAccountManager, error) {
	...
	manager, err := BuildManager(context.Background(), nil, store, NewPeersUpdateManager(nil), nil, "", "netbird.cloud", eventStore, nil, false, MockIntegratedValidator{}, metrics, port_forwarding.NewControllerMock(), settingsMockManager, permissionsManager, false)
	if err != nil {
		return nil, err
	}
