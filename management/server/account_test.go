...
func createManager(t testing.TB) (*DefaultAccountManager, error) {
	...
	manager, err := BuildManager(context.Background(), store, NewPeersUpdateManager(nil), NewJobManager(nil), nil, "", "netbird.cloud", eventStore, nil, false, MockIntegratedValidator{}, metrics, port_forwarding.NewControllerMock(), settingsMockManager, permissionsManager, false)
	if err != nil {
		return nil, err
	}
