...
func TestGetPeerNetworkMap_ProdAccount_CompareImplementations(t *testing.T) {
	...
	legacyNetworkMap := testAccount.GetPeerNetworkMap(ctx, testingPeerID, dns.CustomZone{}, nil, validatedPeersMap, resourcePolicies, routers, nil, groupIDToUserIDs)
	require.NotNil(t, legacyNetworkMap, "GetPeerNetworkMap returned nil")

	components := testAccount.GetPeerNetworkMapComponents(ctx, testingPeerID, dns.CustomZone{}, nil, validatedPeersMap, resourcePolicies, routers, groupIDToUserIDs)
	require.NotNil(t, components, "GetPeerNetworkMapComponents returned nil")
...
func TestGetPeerNetworkMap_ProdAccount_AllPeers(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	for peerID := range testAccount.Peers {
		legacyNetworkMap := testAccount.GetPeerNetworkMap(ctx, peerID, dns.CustomZone{}, nil, validatedPeersMap, resourcePolicies, routers, nil, groupIDToUserIDs)
		require.NotNil(t, legacyNetworkMap, "GetPeerNetworkMap returned nil for peer %s", peerID)

		components := testAccount.GetPeerNetworkMapComponents(ctx, peerID, dns.CustomZone{}, nil, validatedPeersMap, resourcePolicies, routers, groupIDToUserIDs)
		require.NotNil(t, components, "GetPeerNetworkMapComponents returned nil for peer %s", peerID)

		newNetworkMap := CalculateNetworkMapFromComponents(ctx, components)
		require.NotNil(t, newNetworkMap, "CalculateNetworkMapFromComponents returned nil for peer %s", peerID)

		normalizeAndSortNetworkMap(legacyNetworkMap)
		normalizeAndSortNetworkMap(newNetworkMap)

		legacyJSON, err := json.Marshal(legacyNetworkMap)
		require.NoError(t, err, "error marshaling legacy network map for peer %s", peerID)

		newJSON, err := json.Marshal(newNetworkMap)
		require.NoError(t, err, "error marshaling new network map for peer %s", peerID)

		if string(legacyJSON) != string(newJSON) {
			failedPeers = append(failedPeers, peerID)

			outputDir := filepath.Join("testdata", fmt.Sprintf("failed_peer_%s", peerID))
			require.NoError(t, os.MkdirAll(outputDir, 0755))

			legacyIndented, _ := json.MarshalIndent(legacyNetworkMap, "", "  ")
			newIndented, _ := json.MarshalIndent(newNetworkMap, "", "  ")
			componentsIndented, _ := json.MarshalIndent(components, "", "  ")

			_ = os.WriteFile(filepath.Join(outputDir, "legacy_networkmap.json"), legacyIndented, 0644)
			_ = os.WriteFile(filepath.Join(outputDir, "components_networkmap.json"), newIndented, 0644)
			_ = os.WriteFile(filepath.Join(outputDir, "components.json"), componentsIndented, 0644)

			t.Errorf("NetworkMap mismatch for peer %s. Files saved to %s", peerID, outputDir)
		}
	}

	require.Empty(t, failedPeers, "NetworkMap comparison failed for %d peers: %v", len(failedPeers), failedPeers)

	t.Logf("✅ NetworkMaps are identical for all %d peers", len(testAccount.Peers))
}
...
func BenchmarkGetPeerNetworkMapCompactCached(b *testing.B) {
	...
	regularNm := builder.GetPeerNetworkMap(ctx, testingPeerID, customZone, nil, validatedPeersMap, nil)
	...
	components := account.GetPeerNetworkMapComponents(ctx, testingPeerID, customZone, nil, validatedPeersMap, resourcePolicies, routers, agUsers)
	...
	b.Run("Legacy", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = account.GetPeerNetworkMap(ctx, testingPeerID, customZone, nil, validatedPeersMap, resourcePolicies, routers, nil, agUsers)
		}
	})
