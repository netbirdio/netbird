...
func (h *NameserversHandler) CreateNameserverGroup(w http.ResponseWriter, r *http.Request) {
	...
	nsGroup, err := h.accountManager.CreateNameServerGroup(account.Id, req.Name, req.Description, nsList, req.Groups, req.Primary, req.Domains, req.Enabled, user.Id, req.SearchDomains)
	if err != nil {
		util.WriteError(err, w)
		return
	}
...
func toNameserverGroupResponse(serverNSGroup *nbdns.NameServerGroup) *api.NameserverGroup {
	...
	return &api.NameserverGroup{
		...
		SearchDomains: serverNSGroup.SearchDomains,
	}
}
