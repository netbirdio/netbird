...
func Test_RegisterPeerByUser(t *testing.T) {
	...
	am, err := BuildManager(context.Background(), s, NewPeersUpdateManager(nil), NewJobManager(nil), nil, "", "netbird.cloud", eventStore, nil, false, MockIntegratedValidator{}, metrics, port_forwarding.NewControllerMock(), settingsMockManager, permissionsManager, false)
	assert.NoError(t, err)
