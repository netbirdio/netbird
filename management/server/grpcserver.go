...
func (s *GRPCServer) Job(srv proto.ManagementService_JobServer) error {
	...
	// First message from client = hello
	var hello proto.EncryptedMessage
	if err := srv.RecvMsg(&hello); err != nil {
		return status.Errorf(codes.InvalidArgument, "missing hello: %v", err)
	}
	...
	//Start a receiver goroutine to handle job responses from client
	go func() {
		for {
			...
			if _, err := s.parseRequest(ctx, &msg, jobResp); err != nil {
				log.WithContext(ctx).Warnf("failed to parse job response: %v", err)
				continue
			}
			log.WithContext(ctx).Warnf("recv job response: %v", jobResp)
			if err := s.jobManager.HandleResponse(jobResp); err != nil {
				log.WithContext(ctx).Warnf("failed to handle job response: %v", err)
			}
		}
	}()

	// Create per-peer job channel and send jobs as they arrive
	...
	// optional: send initial handshake job if you want
	if err := s.sendInitialJob(ctx, peerKey, srv); err != nil {
		return err
	}

	// Main loop: send jobs; exit when client disconnects
	for {
		select {
		case job, ok := <-updates:
			if !ok {
			...
			if err := s.sendJob(ctx, accountID, peerKey, peer, job, srv); err != nil {
				// log but don't kill the stream; the client may still be connected
...
func (s *GRPCServer) sendJob(
	...
	if err != nil {
		...
		return nil // don't kill the whole stream
	}
