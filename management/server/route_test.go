...
func TestCreateRoute(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			outRoute, err := am.CreateRoute(context.Background(), account.Id, testCase.inputArgs.network, testCase.inputArgs.networkType, testCase.inputArgs.domains, testCase.inputArgs.peerKey, testCase.inputArgs.peerGroupIDs, testCase.inputArgs.description, testCase.inputArgs.netID, testCase.inputArgs.masquerade, testCase.inputArgs.metric, testCase.inputArgs.groups, testCase.inputArgs.accessControlGroups, testCase.inputArgs.enabled, userID, testCase.inputArgs.keepRoute, testCase.inputArgs.isSelected)
...
func TestGetNetworkMap_RouteSyncPeerGroups(t *testing.T) {
	...
	newRoute, err := am.CreateRoute(context.Background(), account.Id, baseRoute.Network, baseRoute.NetworkType, baseRoute.Domains, baseRoute.Peer, baseRoute.PeerGroups, baseRoute.Description, baseRoute.NetID, baseRoute.Masquerade, baseRoute.Metric, baseRoute.Groups, baseRoute.AccessControlGroups, baseRoute.Enabled, userID, baseRoute.KeepRoute, baseRoute.IsSelected)
	require.NoError(t, err)
...
func TestGetNetworkMap_RouteSync(t *testing.T) {
	...
	createdRoute, err := am.CreateRoute(context.Background(), account.Id, baseRoute.Network, baseRoute.NetworkType, baseRoute.Domains, peer1ID, []string{}, baseRoute.Description, baseRoute.NetID, baseRoute.Masquerade, baseRoute.Metric, baseRoute.Groups, baseRoute.AccessControlGroups, false, userID, baseRoute.KeepRoute, baseRoute.IsSelected)
	require.NoError(t, err)
...
func TestRouteAccountPeersUpdate(t *testing.T) {
	...
	t.Run("creating route no routing peer and no peers in groups", func(t *testing.T) {
		...
		_, err := manager.CreateRoute(
			...
			route.Groups, []string{}, true, userID, route.KeepRoute, route.IsSelected,
		)
		require.NoError(t, err)
	...
	t.Run("creating route with a routing peer", func(t *testing.T) {
		...
		newRoute, err := manager.CreateRoute(
			...
			baseRoute.Groups, []string{}, true, userID, baseRoute.KeepRoute, baseRoute.IsSelected,
		)
		require.NoError(t, err)
		baseRoute = *newRoute
	...
	t.Run("adding peer to route peer groups that do not have any peers", func(t *testing.T) {
		...
		_, err := manager.CreateRoute(
			...
			newRoute.Groups, []string{}, true, userID, newRoute.KeepRoute, newRoute.IsSelected,
		)
		require.NoError(t, err)
