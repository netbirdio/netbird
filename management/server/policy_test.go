...
func TestAccount_getPeersByPolicy(t *testing.T) {
	...
	t.Run("check that all peers get map", func(t *testing.T) {
		for _, p := range account.Peers {
			peers, firewallRules := account.getPeerConnectionResources(context.Background(), p.ID, validatedPeers, nil)
	...
	t.Run("check first peer map details", func(t *testing.T) {
		peers, firewallRules := account.getPeerConnectionResources(context.Background(), "peerB", validatedPeers, nil)
...
func TestAccount_getPeersByPolicyDirect(t *testing.T) {
	...
	t.Run("check first peer map", func(t *testing.T) {
		peers, firewallRules := account.getPeerConnectionResources(context.Background(), "peerB", approvedPeers, nil)
		assert.Contains(t, peers, account.Peers["peerC"])
	...
	t.Run("check second peer map", func(t *testing.T) {
		peers, firewallRules := account.getPeerConnectionResources(context.Background(), "peerC", approvedPeers, nil)
...
func TestAccount_getPeersByPolicyPostureChecks(t *testing.T) {
	...
	t.Run("verify peer's network map with default group peer list", func(t *testing.T) {
		...
		peers, firewallRules = account.getPeerConnectionResources(context.Background(), "peerE", approvedPeers, nil)
		assert.Len(t, peers, 4)
		assert.Len(t, firewallRules, 4)
		assert.Contains(t, peers, account.Peers["peerA"])
		assert.Contains(t, peers, account.Peers["peerC"])
		...
		// all source group peers satisfying the NB posture check should establish connection
		peers, firewallRules = account.getPeerConnectionResources(context.Background(), "peerI", approvedPeers, nil)
		assert.Len(t, peers, 4)
		assert.Len(t, firewallRules, 4)
		assert.Contains(t, peers, account.Peers["peerA"])
		assert.Contains(t, peers, account.Peers["peerC"])
	...
	t.Run("verify peer's network map with modified group peer list", func(t *testing.T) {
		...
		peers, firewallRules = account.getPeerConnectionResources(context.Background(), "peerA", approvedPeers, nil)
