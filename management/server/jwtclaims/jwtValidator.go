...
func getPublicKey(token *jwt.Token, jwks *Jwks) (interface{}, error) {
	...
	log.Debugf("Token header KID: %s", token.Header["kid"])
	for k := range jwks.Keys {
		log.Debugf("JWKS Inspecting: %s", jwks.Keys[k].Kid)
		if token.Header["kid"] != jwks.Keys[k].Kid {
			continue
		}
		log.Debugf("Found !: %s", jwks.Keys[k].Kid)
...
func getPublicKeyFromJWK_RSA(jwk JSONWebKey) (*rsa.PublicKey, error) {
	...
	log.Debugf("JWK E: %s", jwk.E)
	log.Debugf("JWK N: %s", jwk.N)
	...
	if err != nil {
		return nil, fmt.Errorf("unable to decode JWK modulus, error: %s", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("unable to decode JWK exponent, error: %s", err)
	}
