...
read_nb_port() {
  READ_NETBIRD_PORT=""
  echo -n "Enter the HTTPS port you want to use for NetBird (default; 443): " > /dev/stderr
  read -r READ_NETBIRD_PORT < /dev/tty
  if [ "$READ_NETBIRD_PORT-x" == "-x" ]; then
    $NETBIRD_PORT=443
  fi    
...
read_nb_http_port() {
  ...
  echo -n "Enter the HTTP port you want to use for NetBird (default; 80): " > /dev/stderr
  read -r READ_NETBIRD_HTTP_PORT < /dev/tty
  if [ "$READ_NETBIRD_HTTP_PORT-x" == "-x" ]; then
...
check_cf_api_key() {
  API_KEY=$1
  if [ "$API_KEY-x" == "-x" ]; then
    echo "The CLOUDFLARE_API_KEY variable cannot be empty." > /dev/stderr
    return 1
  fi
  return 0
}
...
read_cf_api_key() {
  ...
  echo -n "Enter the Cloudflare API Key you want to use for NetBird (e.g. Zjkx6LOp_ccmMIaddzoa1dEkJ-U1-ax2EgasddjB): " > /dev/stderr
  read -r READ_CLOUDFLARE_API_KEY < /dev/tty
  if ! check_cf_api_key "$READ_CLOUDFLARE_API_KEY"; then
    read_cf_api_key
  fi
  echo "$READ_CLOUDFLARE_API_KEY"
}
...
initEnvironment() {
  ...
  CLOUDFLARE_API_KEY=$(read_cf_api_key)
...
renderRelayEnv() {
  cat <<EOF
NB_LOG_LEVEL=info
NB_LISTEN_ADDRESS=:$NETBIRD_HTTP_PORT
