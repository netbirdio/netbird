...
start_services_and_show_instructions() {
  ...
  if [[ "$REVERSE_PROXY_TYPE" == "0" ]]; then
    ...
    if [[ "$ENABLE_PROXY" == "true" ]]; then
      ...
      # Phase 3: Create CrowdSec bouncer key if enabled
      if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
        echo "Registering CrowdSec bouncer..."
        # Wait for CrowdSec to be healthy (healthcheck handles this via depends_on, but be safe)
        local cs_retries=0
      ...
      # Generate proxy.env with the token (and CrowdSec key if enabled)
...
"
  ...
  netbird_proxy_certs:"
    ...
    if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
      crowdsec_service="
  # CrowdSec LAPI - IP reputation and community blocklist
  crowdsec:
...
EOF
  ...
  if [[ "$ENABLE_CROWDSEC" == "true" && -n "$CROWDSEC_BOUNCER_KEY" ]]; then
    cat <<EOF
# CrowdSec IP reputation (LAPI running in crowdsec container)
