...
| `-http-only`     | `NB_PROXY_HTTP_ONLY`             | Serve plain HTTP only, disabling TLS/SNI on the main port. Intended for when the proxy is behind a TLS-terminating reverse proxy   | `false`                                            |
