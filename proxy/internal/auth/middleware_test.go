...
func TestAddDomain_ValidKey(t *testing.T) {
	...
	err := mw.AddDomain("example.com", []Scheme{scheme}, kp.PublicKey, time.Hour, "", "", nil, false, true)
	require.NoError(t, err)
...
func TestAddDomain_EmptyKey(t *testing.T) {
	...
	scheme := &stubScheme{method: auth.MethodPIN, promptID: "pin"}
	err := mw.AddDomain("example.com", []Scheme{scheme}, "", time.Hour, "", "", nil, false, true)
	require.Error(t, err)
...
func TestAddDomain_InvalidBase64(t *testing.T) {
	...
	scheme := &stubScheme{method: auth.MethodPIN, promptID: "pin"}
	err := mw.AddDomain("example.com", []Scheme{scheme}, "not-valid-base64!!!", time.Hour, "", "", nil, false, true)
	require.Error(t, err)
...
func TestAddDomain_WrongKeySize(t *testing.T) {
	...
	scheme := &stubScheme{method: auth.MethodPIN, promptID: "pin"}
	err := mw.AddDomain("example.com", []Scheme{scheme}, shortKey, time.Hour, "", "", nil, false, true)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "invalid session public key size")
...
func TestAddDomain_NoSchemes_NoKeyRequired(t *testing.T) {
	...
	err := mw.AddDomain("example.com", nil, "", time.Hour, "", "", nil, false, true)
...
// A domain management has not marked validated is never registered, so no
// request for it can reach a backend even if the mapping was delivered.
func TestAddDomain_RefusesUnvalidatedDomain(t *testing.T) {
	mw := NewMiddleware(log.StandardLogger(), nil, nil)

	err := mw.AddDomain("unproven.example.com", nil, "", time.Hour, "", "", nil, false, false)
	require.Error(t, err, "an unvalidated domain must be rejected")
	assert.Contains(t, err.Error(), "not validated")

	mw.domainsMux.RLock()
	_, exists := mw.domains["unproven.example.com"]
	mw.domainsMux.RUnlock()
	assert.False(t, exists, "an unvalidated domain must not be registered")
}
...
func TestAddDomain_OverwritesPreviousConfig(t *testing.T) {
	...
	require.NoError(t, mw.AddDomain("example.com", []Scheme{scheme}, kp1.PublicKey, time.Hour, "", "", nil, false, true))
	require.NoError(t, mw.AddDomain("example.com", []Scheme{scheme}, kp2.PublicKey, 2*time.Hour, "", "", nil, false, true))
...
func TestRemoveDomain(t *testing.T) {
	...
	scheme := &stubScheme{method: auth.MethodPIN, promptID: "pin"}
	require.NoError(t, mw.AddDomain("example.com", []Scheme{scheme}, kp.PublicKey, time.Hour, "", "", nil, false, true))
...
func TestProtect_DomainWithNoSchemesPassesThrough(t *testing.T) {
	mw := NewMiddleware(log.StandardLogger(), nil, nil)
	require.NoError(t, mw.AddDomain("example.com", nil, "", time.Hour, "", "", nil, false, true))
...
func TestProtect_PrivateService_TunnelPeerGroupsPropagate(t *testing.T) {
	...
	require.NoError(t, mw.AddDomain("agent.example.com", nil, kp.PublicKey, time.Hour, "acct-1", "svc-1", nil, true, true))
...
func TestProtect_MultipleSchemes(t *testing.T) {
	...
	require.NoError(t, mw.AddDomain("example.com", []Scheme{pinScheme, passwordScheme}, kp.PublicKey, time.Hour, "", "", nil, false, true))
...
func TestAddDomain_RandomBytes32NotEd25519(t *testing.T) {
	...
	err = mw.AddDomain("example.com", []Scheme{scheme}, key, time.Hour, "", "", nil, false, true)
...
func TestAddDomain_InvalidKeyDoesNotCorruptExistingConfig(t *testing.T) {
	...
	err := mw.AddDomain("example.com", []Scheme{scheme}, "bad", time.Hour, "", "", nil, false, true)
	require.Error(t, err)
...
func TestCheckIPRestrictions_UnparseableAddress(t *testing.T) {
	...
	err := mw.AddDomain("example.com", nil, "", 0, "acc1", "svc1",
		restrict.ParseFilter(restrict.FilterConfig{AllowedCIDRs: []string{"10.0.0.0/8"}}), false, true)
	require.NoError(t, err)
...
func TestCheckIPRestrictions_UsesCapturedDataClientIP(t *testing.T) {
	...
	err := mw.AddDomain("example.com", nil, "", 0, "acc1", "svc1",
		restrict.ParseFilter(restrict.FilterConfig{AllowedCIDRs: []string{"203.0.113.0/24"}}), false, true)
	require.NoError(t, err)
...
func TestCheckIPRestrictions_NilGeoWithCountryRules(t *testing.T) {
	...
	err := mw.AddDomain("example.com", nil, "", 0, "acc1", "svc1",
		restrict.ParseFilter(restrict.FilterConfig{AllowedCountries: []string{"US"}}), false, true)
	require.NoError(t, err)
...
func TestCheckIPRestrictions_OverlayOriginRespectsCIDR(t *testing.T) {
	...
	err := mw.AddDomain("example.com", nil, "", 0, "acc1", "svc1",
		restrict.ParseFilter(restrict.FilterConfig{AllowedCIDRs: []string{"100.64.0.0/16"}}), false, true)
	require.NoError(t, err)
...
func TestProtect_OIDCWithOtherMethodShowsLoginPage(t *testing.T) {
	...
	require.NoError(t, mw.AddDomain("example.com", []Scheme{oidcScheme, pinScheme}, kp.PublicKey, time.Hour, "", "", nil, false, true))
...
func TestProtect_HeaderAuth_ForwardsOnSuccess(t *testing.T) {
	...
	require.NoError(t, mw.AddDomain("example.com", []Scheme{hdr}, kp.PublicKey, time.Hour, "acc1", "svc1", nil, false, true))
...
func TestProtect_HeaderAuth_MissingHeaderFallsThrough(t *testing.T) {
	...
	require.NoError(t, mw.AddDomain("example.com", []Scheme{hdr, pinScheme}, kp.PublicKey, time.Hour, "acc1", "svc1", nil, false, true))
