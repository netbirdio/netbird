...
func selectInterfaceForUnspecified() (*interfaceSelection, error) {
	...
	if iface4, err := systemops.GetBestInterface(netip.IPv4Unspecified()); err == nil {
		result.iface4 = iface4
	} else {
		log.Debugf("No IPv4 default route found: %v", err)
	}

	if iface6, err := systemops.GetBestInterface(netip.IPv6Unspecified()); err == nil {
...
func selectInterface(dest netip.Addr) (*interfaceSelection, error) {
	...
	iface, err := systemops.GetBestInterface(dest)
	if err != nil {
...
func setUnicastIf(fd uintptr, network string, selection *interfaceSelection) error {
	...
	case strings.HasSuffix(network, "4"):
		...
		return setUnicastIfIPv4(fd, network, selection)

	case strings.HasSuffix(network, "6"):
		// IPv6 socket (udp6, tcp6, ip6) - could be dual-stack or IPv6-only
		return setUnicastIfIPv6(fd, network, selection)
	}
...
func setUnicastIfIPv4(fd uintptr, network string, selection *interfaceSelection) error {
	...
	log.Debugf("Set IP_UNICAST_IF=%d on %s for %s", selection.iface4.Index, selection.iface4.Name, network)
	return nil
}

func setUnicastIfIPv6(fd uintptr, network string, selection *interfaceSelection) error {
	...
	if isDualStack && selection.iface4 != nil {
		...
		log.Debugf("Set IP_UNICAST_IF=%d on %s for %s (dual-stack)", selection.iface4.Index, selection.iface4.Name, network)
	}
	...
	log.Debugf("Set IPV6_UNICAST_IF=%d on %s for %s", selection.iface6.Index, selection.iface6.Name, network)
	return nil
}
...
func applyUnicastIFToSocket(network string, address string, c syscall.RawConn) error {
	...
	err = c.Control(func(fd uintptr) {
		controlErr = setUnicastIf(fd, network, selection)
	})
