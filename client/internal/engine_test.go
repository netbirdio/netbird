...
func TestEngine_UpdateNetworkMap(t *testing.T) {
	...
	engine.wgInterface, err = iface.NewWGIFace("utun102", "100.64.0.1/24", iface.DefaultMTU, newNet, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEngine_UpdateNetworkMapWithRoutes(t *testing.T) {
	...
	for n, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			engine.wgInterface, err = iface.NewWGIFace(wgIfaceName, wgAddr, iface.DefaultMTU, newNet, nil)
