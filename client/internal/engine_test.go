...
func startManagement(t *testing.T, dataDir, testFile string) (*grpc.Server, string, error) {
	...
	networkMapController := controller.NewController(context.Background(), store, metrics, updateManager, requestBuffer, server.MockIntegratedValidator{}, settingsMockManager, "netbird.selfhosted")
	accountManager, err := server.BuildManager(context.Background(), store, networkMapController, nil, "", eventStore, nil, false, ia, metrics, settingsMockManager, permissionsManager, false)
	if err != nil {
		return nil, "", err
	}
