...
// PasswordRequiredMarker tells Swift to prompt for a password and retry. It is
...
// HostKeyUnknownMarker tells Swift to show the fingerprint and, on confirmation,
...
// marker message the Swift side parses.
...
// inside the network extension process where the tunnel-bound Client lives.
//
...
type SSHClient struct {
	...
	// regular SSH servers in the profile's preferences. Swift supplies them,
...
// authorization page when the target peer requires JWT authentication. The
// extension has no UI of its own, so the opener hands the URL to the main app.
// Must be set before Connect to be effective.
...
// Swift can show the fingerprint and, once confirmed, retry with the key
...
func (s *SSHClient) requestJWTToken(cfg *profilemanager.Config, cfgPath string) (string, error) {
	...
	tokenInfo, err := runSSHOAuthFlow(ctx, flow, urlOpener, func() {
...
// runSSHOAuthFlow drives an already acquired OAuth flow to a token: requests the
// flow info, presents the verification URL through the opener and waits for the
// browser round-trip. Open is called synchronously — it is what marks the
// surface as opened on the client side, and a fast token's OnLoginSuccess is a
// no-op until it has, so the dismissal would be dropped rather than delayed.
// Openers must therefore not block: they post their UI work and return.
// onWaiting, when set, runs after the URL is shown, right before the blocking
// wait.
func runSSHOAuthFlow(ctx context.Context, flow auth.OAuthFlow, urlOpener URLOpener, onWaiting func()) (*auth.TokenInfo, error) {
