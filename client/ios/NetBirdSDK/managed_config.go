...
// Managed configuration key names for Apple Managed App Configuration (AppConfig).
// These match the keys expected in the com.apple.configuration.managed UserDefaults domain.
const (
...
// Key name getters for the Swift client to reference the same key constants.
...
// ManagedConfig holds configuration values pushed by an MDM via Apple Managed App Configuration
// (AppConfig). Values set here override user preferences on every app launch.
//
// The native iOS/tvOS app reads from UserDefaults(suiteName: "com.apple.configuration.managed")
// and populates this struct via the setter methods, then calls Apply() to write the values
// to the config file.
...
// Uses DirectUpdateOrCreateConfig for tvOS sandbox compatibility.
func (m *ManagedConfig) Apply(configPath string) error {
	...
	// Use DirectUpdateOrCreateConfig for tvOS sandbox compatibility
	_, err := profilemanager.DirectUpdateOrCreateConfig(input)
	if err != nil {
