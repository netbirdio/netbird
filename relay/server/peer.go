...
func (p *Peer) sendPeersOnline(onlinePeers []messages.PeerID) {
	msgs, err := messages.MarshalPeersOnline(onlinePeers)
	if err != nil {
		p.log.Errorf("failed to marshal peers online message: %s", err)
		return
	}

	for n, msg := range msgs {
		if _, err := p.Write(msg); err != nil {
			p.log.Errorf("failed to write %d. peers online message: %s", n, err)
		}
	}
}
