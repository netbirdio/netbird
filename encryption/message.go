...
func EncryptMessage(remotePubKey wgtypes.Key, ourPrivateKey wgtypes.Key, message pb.Message) ([]byte, error) {
	...
	if err != nil {
		log.WithContext(ctx).Errorf("failed marshalling message %v", err)
		return nil, err
	}

	encryptedBytes, err := Encrypt(byteResp, remotePubKey, ourPrivateKey)
	if err != nil {
		log.WithContext(ctx).Errorf("failed encrypting SyncResponse %v", err)
		return nil, err
	}
...
func DecryptMessage(remotePubKey wgtypes.Key, ourPrivateKey wgtypes.Key, encryptedMessage []byte, message pb.Message) error {
	...
	if err != nil {
		log.WithContext(ctx).Warnf("error while decrypting Sync request message from peer %s", remotePubKey.String())
		return err
	}

	err = pb.Unmarshal(decrypted, message)
	if err != nil {
		log.WithContext(ctx).Warnf("error while umarshalling Sync request message from peer %s", remotePubKey.String())
		return err
	}
	return nil
}
